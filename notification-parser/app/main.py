...
PLATFORM_PATTERNS = {
    ...
    "bocom": {  # 交通银行（95559，太平洋卡/借记卡）
        "keywords": ["交通银行", "交行", "太平洋卡"],
        "amount_patterns": [
            r"(?:消费|支出|付款|转账|还款|退款|转入|收入|缴纳|缴费)\D*?人民币\s*(\d{1,10}(?:,\d{3})*(?:\.\d{1,2})?)",
            r"(?:消费|支出|付款|转账|还款|退款|转入|收入|缴纳|缴费)\D*?[¥￥]?\s*(\d{1,10}(?:\.\d{1,2})?)",
            r"交易金额[：:]\s*[¥￥]?\s*(\d{1,10}(?:\.\d{1,2})?)",
            r"(\d{1,10}(?:\.\d{1,2})?)\s*元",
        ],
        "desc_patterns": [
    ...
    "citic": {  # 中信银行（关键词+通用金额兜底）
        "keywords": ["中信银行", "中信", "CITIC"],
        "amount_patterns": [
            r"(?:消费|支出|付款|转账|还款|退款|转入|收入)\D*?人民币\s*(\d{1,10}(?:,\d{3})*(?:\.\d{1,2})?)",
            r"(?:消费|支出|付款|转账|还款|退款|转入|收入)\D*?[¥￥]?\s*(\d{1,10}(?:\.\d{1,2})?)",
            r"(\d{1,10}(?:\.\d{1,2})?)\s*元",
    ...
    "unionpay": {  # 云闪付/银联
        "keywords": ["云闪付", "UnionPay", "银联"],
        "amount_patterns": [
            r"(?:付款|消费|支付|转账|收款|退款)\D*?[¥￥]?\s*(\d{1,10}(?:\.\d{1,2})?)\s*元",
            r"[¥￥]\s*(\d{1,10}(?:\.\d{1,2})?)",
            r"(\d{1,10}(?:\.\d{1,2})?)\s*元",
        ],
        "desc_patterns": [
    ...
    "meituan": {  # 美团（外卖/到店/买菜，多为餐饮）
        "keywords": ["美团", "美团外卖", "美团买菜", "大众点评"],
        "amount_patterns": [
            r"(?:消费|支出|付款|支付|订单)\D*?[¥￥]?\s*(\d{1,10}(?:\.\d{1,2})?)\s*元",
            r"[¥￥]\s*(\d{1,10}(?:\.\d{1,2})?)",
            r"(\d{1,10}(?:\.\d{1,2})?)\s*元",
        ],
        "desc_patterns": [
    ...
    "jd": {  # 京东（商城订单/白条，多为购物）
        "keywords": ["京东", "JD.com", "京东白条", "京东支付"],
        "amount_patterns": [
            r"(?:订单|支付|消费|支出|付款)\D*?[¥￥]?\s*(\d{1,10}(?:\.\d{1,2})?)\s*元",
            r"[¥￥]\s*(\d{1,10}(?:\.\d{1,2})?)",
            r"订单金额[：:]\s*[¥￥]?\s*(\d{1,10}(?:\.\d{1,2})?)",
            r"(\d{1,10}(?:\.\d{1,2})?)\s*元",
        ],
        "desc_patterns": [
            r"订单(?:号)?[：:\s]*\S+\s*(.+?)(?:\s|，|,|$)",
...
def detect_platform(text: str, source: Optional[str] = None) -> str:
    ...
    source 是推送端已标定的平台 id（如 bocom），精确命中直接采用；
    否则按关键词扫描。注意不能先做子串匹配——"bocom" 里包含 "boc"，
    顺序扫描会把交通银行误判成中国银行。
    """
    if source and source.lower() in PLATFORM_PATTERNS:
        return source.lower()
...
def detect_type(text: str) -> str:
    ...
    显式支出动词优先：银行短信里“利息支出”“工资卡消费”这类名词+动词组合，
    动词才是方向信号（之前先查收入词会把“贷款利息支出”误判为收入）。
    注意先剥掉“付款方/收款方”等角色词，避免把身份名词当动作。
    """
    clean = text
    for role in ("付款方", "收款方", "付款人", "收款人", "转账方"):
        clean = clean.replace(role, "")
    expense_strong = [
        "支出", "消费", "付款", "扣款", "转出", "缴费", "缴纳", "还款",
    ]
    for kw in expense_strong:
        if kw in clean:
            return "expense"

    # 再检查收入关键词（收入信号明确时，如“工资到账”“利息到账”）
    income_keywords = [
        "收入", "到账", "转入", "退款", "红包", "收款",
        "回款", "工资", "奖金", "利息", "分红", "报销",
    ]
