  ...
  <div class="mx-auto min-w-0 w-full max-w-5xl px-4 py-6">
    ...
    <div class="flex flex-wrap items-center gap-3">
      <div class="mr-auto min-w-0">
        <h1 class="text-xl font-semibold" style="color: var(--text-primary)">AI 分析</h1>
    ...
    <div v-else class="mt-6 space-y-6">
      <div v-if="analysis.consumption || analysis.investment || analysis.suggestion" class="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <UCard :style="{ background: 'var(--bg-secondary)', border: '1px solid var(--border)' }" :ui="{ body: 'p-5' }">
          <div class="mb-2 flex items-center gap-2">
            ...
            <h2 class="text-base font-semibold" style="color: var(--text-primary)">消费分析</h2>
          </div>
          <div class="markdown-body text-sm leading-relaxed" style="color: var(--text-secondary)" v-html="renderMd(analysis.consumption)"></div>
        </UCard>
        <UCard :style="{ background: 'var(--bg-secondary)', border: '1px solid var(--border)' }" :ui="{ body: 'p-5' }">
          <div class="mb-2 flex items-center gap-2">
            <AppIcon icon="TrendUp" :size="20" style="color: var(--accent)" />
            <h2 class="text-base font-semibold" style="color: var(--text-primary)">投资分析</h2>
          </div>
          <div class="markdown-body text-sm leading-relaxed" style="color: var(--text-secondary)" v-html="renderMd(analysis.investment)"></div>
        </UCard>
        <UCard :style="{ background: 'var(--bg-secondary)', border: '1px solid var(--border)' }" :ui="{ body: 'p-5' }">
          <div class="mb-2 flex items-center gap-2">
            <AppIcon icon="BookOpen" :size="20" style="color: var(--accent)" />
            <h2 class="text-base font-semibold" style="color: var(--text-primary)">综合建议</h2>
          </div>
          <div class="markdown-body text-sm leading-relaxed" style="color: var(--text-secondary)" v-html="renderMd(analysis.suggestion)"></div>
        </UCard>
      </div>
      ...
      <UCard
        ...
        :ui="{ body: 'p-5' }"
      >
        <h3 class="mb-3 text-base font-semibold" style="color: var(--text-primary)">消费分类</h3>
        <div class="flex flex-wrap items-center gap-6">
          <div ref="pieEl" class="h-[220px] w-[220px] min-w-[220px] shrink-0" role="img" aria-label="消费分类占比图"></div>
      ...
      <UCard
        ...
        :ui="{ body: 'p-5' }"
      >
        <h2 class="mb-3 text-base font-semibold" style="color: var(--text-primary)">历史分析</h2>
        <div class="space-y-3">
          <div v-for="batch in history" :key="batch.created_at">
            <div class="mb-1 text-xs tabular-nums" style="color: var(--text-secondary)">{{ formatTime(batch.created_at) }}</div>
            <div class="space-y-1.5">
              <button
                v-for="item in batch.items"
                :key="item.id"
                type="button"
                class="legend-btn flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left"
              ...
              >
                ...
                <span class="min-w-0 flex-1 truncate text-xs" style="color: var(--text-secondary)">{{ stripMd(item.content).substring(0, 80) }}...</span>
              </button>
            </div>
          </div>
        </div>
      ...
      >
        ...
        <p class="mt-3 text-sm font-medium" style="color: var(--text-primary)">还没有分析结果</p>
        <p class="mt-1 text-sm" style="color: var(--text-secondary)">点右上角「运行分析」，先出一份看看。</p>
        <UButton class="mt-4" :loading="analyzing" @click="analyze">运行分析</UButton>
      </UCard>
    </div>
  </div>
</template>
