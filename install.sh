...
if [ -f ".env" ]; then
...

  {
    ...
    echo "# 三个分析角色的 OpenClaw agent id（换成你自己的，默认 generic 角色）"
    echo "# OPENCLAW_AGENT_CONSUMPTION=financial_director"
    echo "# OPENCLAW_AGENT_INVESTMENT=investment_director"
    echo "# OPENCLAW_AGENT_SUGGESTION=main"
    echo ""
    echo "# ===== CORS / 部署 ====="
